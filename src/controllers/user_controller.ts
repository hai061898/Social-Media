...
export const createUser = async (
  ...
  try {
    ...
    await sendEmailVerify(
      "Verification code",
      email,
      `<h1> Social  </h1><hr> <b>${randomNumber} </b>`
    );
...
export const verifyEmail = async (
  ...
  try {
    ...
    const [codedb] = await conn.query<RowDataPacket[]>(
      "SELECT token_temp FROM users WHERE email = ? LIMIT 1",
      [req.params.email]
    );
    ...
    await conn.query(
      "UPDATE users SET email_verified = ?, token_temp = ? WHERE email = ?",
      [true, "", req.params.email]
    );
