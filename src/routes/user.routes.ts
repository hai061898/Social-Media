...
import { uploadsCover, uploadsProfile } from "../helpers/multer";
...
const router = Router();
    ...
    router.put(
    "/user/update-cover",
    [verifyToken, uploadsCover.single("cover")],
    user.updatePictureCover
    );
    router.put(
    "/user/update-image-profile",
    [verifyToken, uploadsProfile.single("profile")],
    ...
    router.put(
    "/user/change-account-privacy",
    verifyToken,
    user.changeAccountPrivacy
    );

    router.get("/user/get-search-user/:username", verifyToken, user.getSearchUser);
    router.get(
    "/user/get-another-user-by-id/:idUser",
    verifyToken,
    user.getAnotherUserById
    );
    router.post("/user/add-new-friend", verifyToken, user.AddNewFollowing);
    router.post(
    "/user/accept-follower-request",
    verifyToken,
    user.AcceptFollowerRequest
    );
    router.delete(
    "/user/delete-following/:idUser",
    verifyToken,
    ...
    router.delete(
    "/user/delete-followers/:idUser",
    verifyToken,
    user.deleteFollowers
    );
